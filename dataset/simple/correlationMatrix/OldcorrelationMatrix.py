...
# load images
objectA = np.load('stkA.npy')  #(31, 320, 320)
...
def correlate_and_combine(
    ...
    # initialize 
    correlation_matrix = np.zeros((total_size_y, total_size_x))
    ...
    for iy, y in enumerate(ys):
        for ix, x in enumerate(xs):
            ...
            correlation_matrix[start_y: end_y, start_x: end_x] = cross_corr
...
# =========modified=============
    ...
    return correlation_matrix, dxs, dys
...
for i in range(objectA.shape[0]):  # Iterate over possible dz values
    ...
    correlation_matrix,dxs, dys = correlate_and_combine(
        img1=img1, 
        img2=img2, 
    ...
    correlation_stack.append(correlation_matrix)
...
Y, X = np.meshgrid(np.arange(correlation_matrix.shape[0]), np.arange(correlation_matrix.shape[1]))
